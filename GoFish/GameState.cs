namespace GoFish
{
    public class GameState
    {
        ...
        public Player RandomPlayer(Player currentPlayer) =>
            Players
            .Where(p => !p.Equals(currentPlayer))
        ...
        public string PlayRound(Player player, Player playerToAsk, Values valueToAskFor, Deck stock)
        {
            ...
            var askedCards = playerToAsk.DoYouHaveAny(valueToAskFor, stock);
            if (askedCards.Count() > 0)
            {
                player.AddCardsAndPullOutBooks(askedCards);
                message += $"{playerToAsk.Name} has {askedCards.Count()}"
                + $" {valueToAskFor} card{Player.S(askedCards.Count())}";
            }
